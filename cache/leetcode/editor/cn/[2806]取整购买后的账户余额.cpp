...
public:
    int accountBalanceAfterPurchase(int purchaseAmount) {
        int purchase = 0, min_dist = 101;
        for (int i = 0; i <= 100; i += 10) {
            if (min_dist > abs(purchaseAmount - i)) {
                min_dist = abs(purchaseAmount - i);
                purchase = i;
            } else if (min_dist == abs(purchaseAmount - i)) {
                purchase = max(purchaseAmount, i);
            }
        }
        return 100 - purchase;
    }
